...
double evaluate (double *p, int f_number,int D ){             
	...
	if (f_number == 7)
	{
		...
		for (int i = 1; i <= D; i++)
		{
			outcome = outcome + (i)* (pow(*(p+i-1),4));// + (double(rand() ) / RAND_MAX  ) ;
		}
	...
	for(;repeat < REPEAT; repeat++){
		...
		while (countDE1 < gen_max) {                      
			...
			for (int  i = 0; i < NP; i++) {
				...
				double temp_F = x1[i][D];
				double temp_CR = x1[i][D+1];

				if ((rand2) < 0.1)
				{
					temp_F = 0.1 + double (rand()) / RAND_MAX * 0.9;
				}
				else{
					temp_F = x1[i][D];
				}
				...
				//DE_Mutation
				for (int t = 0; t < D; t++) {
					Mut[t] = x1[a][t] + (temp_F) * (x1[b][t] - x1[c][t]);           
				...
				//DE_Crossover
				for (int k = 0; k < D; k++) {
					if ( float (rand()) / RAND_MAX < (temp_CR) || k == j) {
						trial[k] = Mut[k];
					}
					else                                                     
					{
	 ...
	 cout << "   " << repeat << "   "<< "DE   "  << temp_deta<<"         //        "; 
		...
		while (countDE2 < gen_max ) {                 
			for (int  i = 0; i < NP; i++) {
				...
				double rand2 = double (rand()) / RAND_MAX;
				double rand4 = double (rand()) / RAND_MAX;
				double temp_F = x2[i][D];
				double temp_CR = x2[i][D+1];
				...
				*/
				...
				//DE(BA)_Mutation
				for (int t = 0; t < D; t++) {
					Mut[t] = x2[a][t] + (temp_F) * (x2[b][t] - x2[c][t])  ;           
					if (Mut[t] >= up || Mut[t] <= down )  {
						Mut[t] = x2[a][t];
					}
				}
